...
static void write_to_client(struct connection *conn) {
  ...
  //DBG(("%p Written %d of %d(%d): [%.*s ...]",conn, n, io->len, io->size, 40, io->buf));
  //yxdoit
  DBG(("client_sock %d  Written %d of %d(%d)",conn->client_sock, n, io->len, io->size));
  if (is_error(n)) {
...
static void process_request(struct connection *conn) {
  ...
  //below is means the connect is firstly arrive? yuxiang 
 ...
 DBG(("yx coming 14 \n"));//yx do it 
  ...
  DBG(("yx coming 2:conn->endpoint_type=%d  \n",conn->endpoint_type));//yx do it 
#ifndef NO_CGI
